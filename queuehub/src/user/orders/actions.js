window.trackOrder=(restaurantId,ticketNumber,options={})=>{
  ...
  if(QueueHubVisitorRepository.visitor().notificationsEnabled)QueueHubWebPush?.registerOrder(result.order).catch(error=>{QueueHubDiagnostics?.count('push_watch_register_error');console.warn('[QueueHub] background watch registration failed',error)});
  toast(`${result.restaurant.name} #${ticketNumber} 已加入我的取餐`);if(options.navigate!==false)go('/my-orders');return true;
};
window.completeOrder=id=>{const order=QueueHubVisitorRepository.orders().find(o=>o.id===id);if(!QueueHubOrderCommands.complete(id))return;if(order)QueueHubWebPush?.removeOrder(order).catch(error=>{QueueHubDiagnostics?.count('push_watch_remove_error');console.warn('[QueueHub] background watch removal failed',error)});toast('已標記完成');render()};
window.removeOrder=id=>{const order=QueueHubVisitorRepository.orders().find(o=>o.id===id);if(!QueueHubOrderCommands.remove(id))return;if(order)QueueHubWebPush?.removeOrder(order).catch(error=>{QueueHubDiagnostics?.count('push_watch_remove_error');console.warn('[QueueHub] background watch removal failed',error)});toast('已移除取餐單');render()};
