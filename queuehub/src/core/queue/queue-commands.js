const QueueHubQueueCommands={
  mutate(id,change){if(!getRestaurant(id))return{ok:false,reason:'restaurant-not-found'};return QueueHubStateRepository.update(s=>{const r=s.restaurants.find(x=>x.id===id);const event=change(r)||{};r.updated=Date.now();appendQueueEvent(s,r,event.type,event.number);return{ok:true,restaurant:r,event}},{notify:true,renderAfter:true})},
