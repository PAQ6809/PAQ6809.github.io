...
async function sendMagicLink(event){event.preventDefault();if(!state.supabase){toast('雲端登入模組尚未載入。','error');return;}const email=document.querySelector('#auth-email').value.trim();const btn=document.querySelector('#send-magic-link');btn.disabled=true;try{const {error}=await state.supabase.auth.signInWithOtp({email,options:{emailRedirectTo:CONFIG.baseUrl+'#settings',shouldCreateUser:true}});if(error)throw error;toast('登入連結已寄出，請檢查信箱。','success');document.querySelector('#auth-dialog').close();}catch(err){toast(`寄送失敗：${err.message||'請稍後再試'}`,'error');}finally{btn.disabled=false;}}
