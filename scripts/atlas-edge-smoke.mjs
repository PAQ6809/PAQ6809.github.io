...
assert(searchZero.response.status === 200 && Array.isArray(searchZero.body.results) && searchZero.body.results.length === 0, 'known-zero search returns zero results without error', searchZero.body);
...
const privateHost = await request('import:private-ipv4', '?action=import', { method: 'POST', body: JSON.stringify({ url: 'https://127.0.0.1/' }) });
...
assert(localhost.response.status === 400 && localhost.body.code === 'PRIVATE_HOST_NOT_ALLOWED', 'localhost is rejected', localhost.body);
...
const oversized = await request('import:request-too-large', '?action=import', { method: 'POST', body: JSON.stringify({ url: 'https://example.com/', padding: 'x'.repeat(5000) }) });
...
const blockedOrigin = await request('health:blocked-origin', '?action=health', { headers: { Origin: 'https://not-allowed.invalid' } });
