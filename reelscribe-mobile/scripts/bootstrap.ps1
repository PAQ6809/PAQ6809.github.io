...
Write-Host "安裝固定版本的 JavaScript 相依套件，不執行第三方 lifecycle scripts..."
npm install --ignore-scripts --no-audit --no-fund --silent

Write-Host "完成。下一步："
Write-Host "1. 先執行 npm run check"
Write-Host "2. Android 可執行 npm run android 進行 Debug 真機測試"
Write-Host "3. iOS 必須在 macOS 執行 cd ios; pod install，再用 Xcode 真機編譯"
Write-Host "4. 確認 Bundle ID / applicationId 尚未被其他 App 使用"
Write-Host "5. .do-not-ship 存在期間禁止送審或發布正式版"
