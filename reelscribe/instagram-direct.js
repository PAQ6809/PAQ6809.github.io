...
(() => {
  ...
  if (!input || !resolveButton || !status) return;
  ...
  function parseYouTube(raw) {
    try {
      ...
      if (host === "youtu.be") videoId = url.pathname.split("/").filter(Boolean)[0] || "";
        ...
        videoId = url.searchParams.get("v") || url.pathname.match(/^\/(?:shorts|embed|live)\/([A-Za-z0-9_-]{11})/)?.[1] || "";
      }
  ...
  function updateProvider(state, detail) {
    ...
    if (!log) return;
    log.hidden = false;
    let chip = [...log.children].find((node) => node.dataset.name === "YouTube 相容字幕");
    if (!chip) {
      chip = document.createElement("span");
      chip.dataset.name = "YouTube 相容字幕";
      log.appendChild(chip);
    }
    chip.className = `provider-chip ${state}`;
    chip.textContent = `YouTube 相容字幕 · ${detail}`;
  }
  ...
  async function callCaptions(parsed) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 45_000);
    try {
      const response = await fetch(`${API_BASE}/api/youtube-captions`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Accept": "application/json" },
        body: JSON.stringify({
      ...
      if (!response.ok || !data?.ok || !Array.isArray(data.segments) || !data.segments.length) {
        throw new Error(data?.message || "YouTube 沒有提供可匿名讀取的公開字幕。");
      }
      return data;
    } catch (error) {
      if (error?.name === "AbortError") throw new Error("YouTube 字幕解析逾時。");
      throw error;
    } finally {
      clearTimeout(timer);
    }
  ...
  function applyCaptions(data) {
    ...
    app.clearFile?.();
    const label = data.autoGenerated ? "YouTube 自動字幕" : "YouTube 人工字幕";
    const applied = app.mergeExternalSegments(data.segments, {
      sourceLabel: label,
      replaceOverlapping: false,
    });
    if (!applied) throw new Error("YouTube 字幕內容是空的。");
    if (resultStats) {
      const words = String(data.text || "").replace(/\s/g, "").length;
      resultStats.textContent = `${formatDuration(data.duration)} · ${data.segments.length} 段 · 約 ${words} 字 · ${label} · ${data.language || "自動語言"}`;
    }
  }

  function runUniversalFallback() {
    bypassNext = true;
    resolveButton.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
  }

  async function runYouTubeDirect(rawUrl = input.value) {
    if (running) return true;
    const parsed = parseYouTube(rawUrl);
    if (!parsed) return false;
    ...
    if (verifiedActions) verifiedActions.hidden = false;
    resolveButton.disabled = true;
    resolveButton.textContent = "解析 YouTube…";
    setStatus("正在查詢 YouTube 人工字幕與自動字幕…", "ok");
    updateProvider("running", "查詢中");

    try {
      const data = await callCaptions(parsed);
      applyCaptions(data);
      updateProvider("ok", data.autoGenerated ? "自動字幕" : "人工字幕");
      setStatus(`已取得 ${data.autoGenerated ? "YouTube 自動字幕" : "YouTube 人工字幕"}，不需下載影片。`, "ok");
      return true;
    } catch (error) {
      console.warn("YouTube backend captions failed; trying public providers", error);
      updateProvider("fail", "改用公開節點");
      setStatus("相容字幕來源暫時不可用，正在改查 YouTube 公開節點…", "ok");
      return false;
    } finally {
      running = false;
      resolveButton.disabled = false;
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (!parseYouTube(input.value)) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const handled = await runYouTubeDirect();
    if (!handled) setTimeout(runUniversalFallback, 0);
  }, true);

  document.addEventListener("keydown", async (event) => {
    if (event.target !== input || event.key !== "Enter" || !parseYouTube(input.value)) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const handled = await runYouTubeDirect();
    if (!handled) setTimeout(runUniversalFallback, 0);
  ...
  window.ReelScribeYouTubeDirect = Object.freeze({ resolve: runYouTubeDirect, parse: parseYouTube });
})();