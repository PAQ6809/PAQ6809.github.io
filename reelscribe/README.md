...
1. 使用者貼上或從手機分享公開影片連結。
2. Instagram Reel、影片貼文與 IGTV 先走專用雙層解析：輕量公開端點／頁面解析，失敗後再使用最新版 yt-dlp 與瀏覽器模擬相容層。
...
- Service Worker App Shell 快取與 PWA Share Target。
...
- Instagram 專用解析器、腳本順序、無 Cookie／no-referrer、300 MB 上限及 Vercel `/api/health`。
